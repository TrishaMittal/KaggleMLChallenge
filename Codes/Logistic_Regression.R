...
# Methodology: Logistic Regression in R  
...
model <- glm(label ~ alchemy_category_score + alchemy_category + news_front_page, train, family= binomial(link=logit))
...
write.csv(submit, "../Run_Output_Files/OUTPUT_GLR.csv", row.names = F)
cat("model result:\n")
print (model)	
